...
TEST(create_word_list, commandOnly) {
  ...
  EXPECT_EQ(std::string(get_content(word_list)->content), std::string("cat"));
...
TEST(create_word_list, commandWithOneOption) {
  ...
  EXPECT_EQ(std::string(get_content(word_list)->content), std::string("cat"));
  EXPECT_EQ(std::string(get_content(word_list->next)->content),
            std::string("-e"));
...
TEST(create_word_list, commandWithTwoOptions) {
  ...
  EXPECT_EQ(std::string(get_content(word_list)->content), std::string("grep"));
  EXPECT_EQ(std::string(get_content(word_list->next)->content),
            std::string("-rn"));
  EXPECT_EQ(std::string(get_content(word_list->next->next)->content),
            std::string("hello"));
